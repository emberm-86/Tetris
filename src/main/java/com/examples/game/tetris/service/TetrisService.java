...
/**
 * Service class to check if the current tetromino is movable or if a row is completed and if it is
 * completed then the score must be updated.
 */
public class TetrisService {
   ...
   */
  public boolean isActShapeNotLanded(AbstractShape abstractShape, Color[][] fillingColors) {
    ...
    for (int i = 0; i < RECT_NUM; i++) {
      ...
      if (y >= 0 && x < COL_NUM && y < ROW_NUM - 1) {
        isLanded = isLanded || fillingColors[x][y + 1] != BG_COLOR;
      }
    }
  ...
  /**
   * Check if the new tetromino must be drawn before the game session is over. Partial tetromino
   * will not be drawn.
   */
  public boolean isActShapeCannotBeLanded(AbstractShape abstractShape, Color[][] fillingColors) {
    ...
    for (int i = 0; i < RECT_NUM; i++) {
      ...
      if (y >= 0 && x < COL_NUM && y < ROW_NUM) {
        cannotBeLanded = cannotBeLanded || fillingColors[x][y] != BG_COLOR;
      }
    }
  ...
  /**
   * In repaint method it is checked, if a rectangle is a part of the actual tetromino.
   */
  public boolean isInActShape(AbstractShape.Point[] points, int i, int j) {
    boolean inActShape = false;

    for (int k = 0; k < RECT_NUM; k++) {
      inActShape = inActShape || (points[k].x == i && points[k].y == j);
    }
    return inActShape;
  }
   ...
   */
  public void moveLeft(AbstractShape abstractShape, Color[][] fillingColors) {
    AbstractShape.Point[] points = abstractShape.getPoints();
    ...
    for (int i = 0; i < RECT_NUM; i++) {
      ...
      if (x > 0 && y >= 0 && y < ROW_NUM) {
        movableToLeft = movableToLeft && fillingColors[x - 1][y] == BG_COLOR;
      }
    }
   ...
   */
  public void moveRight(AbstractShape abstractShape, Color[][] fillingColors) {
    AbstractShape.Point[] points = abstractShape.getPoints();
    AbstractShape.Point rotationPoint = abstractShape.getRotationPoint();
    ...
    for (int i = 0; i < RECT_NUM; i++) {
      ...
      if (y >= 0 && x < COL_NUM - 1 && y < ROW_NUM) {
        movableToRight = movableToRight && fillingColors[x + 1][y] == BG_COLOR;
      }
    }
