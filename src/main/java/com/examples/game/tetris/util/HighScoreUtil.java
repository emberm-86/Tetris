...
/**
 * Util class for maintaining the high scores. They are stored under the local folder in a .csv. The
 * parsing of the .csv is simplified, but it is enough in this case.
 */
public class HighScoreUtil {
  ...
  /**
   * If a game session is over and the user is new on the high score list, then he can enter his
   * name in a popup dialog and the result can be saved into a .csv file.
   */
  public static void showNewHighScoreEntryDialog(JRootPane rootPane, int score) {
    if (score == 0) {
      return;
    }
    ...
    Comparator<String[]> comparator =
        (o1, o2) -> Integer.valueOf(o2[1]).compareTo(Integer.valueOf(o1[1]));
    ...
    if (highScores.length == HIGH_SCORE_LIST_SIZE && !isBiggerThanLastOnScoreList) {
      return;
    }
    ...
    CustomDialog customDialog =
        new CustomDialog(rootPane, title, new String[]{"Save", "Cancel"}, labelText, inputName);
    ...
    List<String[]> highScoreList = Arrays.stream(highScores).collect(Collectors.toList());
    highScoreList.add((new String[]{inputName.getText(), "" + score}));
    highScoreList.sort(comparator);

    String[][] updatedHighScores = highScoreList.stream().limit(HIGH_SCORE_LIST_SIZE)
        .toArray(String[][]::new);
  ...
  public static void showHighScores(JRootPane rootPane) {
    ...
    CustomDialog customDialog =
        new CustomDialog(rootPane, title, new String[]{"OK"}, null, (JComponent[]) null);
  ...
  private static String[][] readHighScoreList() {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(csv))) {
      return br.lines().skip(1).map(x -> x.split(CSV_PARSE_REGEX)).toArray(String[][]::new);
    } catch (IOException e) {
      throw new RuntimeException(e.getLocalizedMessage(), e);
    }
  }
